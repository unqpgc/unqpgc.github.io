...
const Opcode OP_push = 18;
const Opcode OP_pop = 19;
const Opcode OP_call = 20;
const Opcode OP_icall = 21;
const Opcode OP_return = 22;
...
string opcode_paramTypes(Opcode op) {
	switch (op) {
		...
		case OP_push: return "r";
		case OP_pop: return "r";
...
private:
	...
	void _push(const Reg& r);
	void _pop(const Reg& r);
...
void VM::_step() {
	...
	switch (instr.opcode()) {
		...
		case OP_mod:
			...
			break;
		case OP_push:
			_push(instr.regParam(0));
			break;
		case OP_pop:
			_pop(instr.regParam(0));
			break;
		case OP_call:
...
void VM::_store(const Reg& r1, u64 i, const Reg& r2) {
	...
	cell[i] = _registers[r2];
}

void VM::_print_char(const Reg& r) {
	Value v = _registers[r];
...
void VM::_jump_eq(const Reg& r1, const Reg& r2, const Label& l) {
	...
	if (_registers[r1].vi64() == _registers[r2].vi64()) {
		_jump(l);
	}
}
...
void VM::_add(const Reg& r1, const Reg& r2, const Reg& r3) {
	...
	i64 a = _registers[r2].vi64();
	i64 b = _registers[r3].vi64();
	_registers[r1] = Value(VInt, a + b);
}
...
void VM::_push(const Reg& r) {
	_stack.push_back(_registers[r]);
}

void VM::_pop(const Reg& r) {
	if (_stack.size() == 0) {
		_fail("pop: Empty stack.");
	}
	_registers[r] = _stack.back();
	_stack.pop_back();
}

void VM::_call(const Label& l) {
	_stack.push_back(Value(VLoc, _ip));
	_ip = _program.labelLocation(l);
}
...
void VM::_return() {
	...
	Value v = _stack.back();
	if (v.tag() != VLoc) {
		_fail("return: Top of stack is not a location.");
	}
	_stack.pop_back();
	_ip = v.vu64();
}
